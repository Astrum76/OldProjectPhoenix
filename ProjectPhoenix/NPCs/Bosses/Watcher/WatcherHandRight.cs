...
{
    public class WatcherHandRight : ModNPC
    {
        float speed = 45;
        float turnRes = 30f;
        ...
        private void Col()
        {
            ...
            for (int i = 0; i < Main.npc.Length-1; i++)
            {
                ...
                if (Main.npc[i].type != ModContent.NPCType<WatcherHandRight>() && (Main.npc[i].Distance(npc.Center) <= 128))
                {
                    //Main.NewText("loop lol");

                    Main.npc[i].StrikeNPC(10, 1, (int)npc.rotation);
                }
            }
